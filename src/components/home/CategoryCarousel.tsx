...
const CategoryCarousel: FC<CategoryCarouselProps> = async ({ options }) => {
  try {
    ...
    return (
      <section className="pt-8 sm:pt-12 lg:pt-20">
        <div className="mx-auto mb-10 w-auto text-center md:max-w-4.5xl md:px-36">
        ...
        <div className="w-full overflow-x-auto overflow-y-hidden">
          <ul className="m-0 grid grid-cols-1 gap-7 p-0 xss:grid-cols-2 sm:grid-cols-3">
            {topCategories.map((category: any) => (
              <li
                key={category.id}
                className="relative aspect-498/665 w-full max-w-[498px] flex-none overflow-hidden rounded-[18px]"
              >
                <Link
                ...
                >
                  <GridTileImage
                    ...
                    className="relative overflow-hidden rounded-[18px] object-cover transition duration-300 ease-in-out group-hover:scale-105"
                    label={{
